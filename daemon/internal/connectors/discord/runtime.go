...
func conformanceProfileForEvidence(cfg Config, setup HostedSetup, validated bool, declaredAt time.Time) baseconnectors.CapabilityProfile {
	...
	return baseconnectors.CapabilityProfile{
		...
		GroupRoomCapabilities: baseconnectors.GroupRoomCapabilities{
			MentionEvidence:           supportFlag(cfg.RequireMention),
			AllowlistEvidence:         supportFlag(len(cfg.AllowedGuildIDs) > 0 || len(cfg.AllowedChannelIDs) > 0),
		...
		HandoffCapabilities: baseconnectors.HandoffCapabilities{
			SourceSupport:                 supportFlag(len(cfg.AllowedGuildIDs) > 0 || len(cfg.AllowedChannelIDs) > 0),
			DestinationSupport:            supportFlag(len(cfg.AllowedGuildIDs) > 0 || len(cfg.AllowedChannelIDs) > 0),
