---
name: kura-test-env
description: Use when working on Kura locally and you need to start, inspect, debug, or validate the daemon without touching production state. Default to the test environment at ~/.kura-test and port 19192 unless the user explicitly requests production.
---
...
2. If the listener is an existing Kura daemon for the same environment, stop it first and then restart.
...
6. Use `~/.kura-test/config.json` for connector and provider edits.
7. Leave `~/.kura/config.json` alone unless the user explicitly asked for production work.
...
- If it is a Kura daemon, stop the old listener and restart cleanly.
- If it is not a Kura daemon, treat it as a real conflict and do not kill it without user approval.
