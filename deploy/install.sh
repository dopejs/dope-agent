...
#   KURA_ENV=prod|test          --env <e>        default: prod
#   KURA_BIN_DIR=<dir>          --bin-dir <d>    default: ~/.local/bin
#   KURA_DATA_DIR=<dir>         --data-dir <d>   default: ~/.kura (prod) / ~/.kura-test (test)
#   KURA_BIND_ADDR=<host:port>  --bind <a>       default: 127.0.0.1:19191 (prod) / :19192 (test)
#   KURA_LOG_LEVEL=<level>                       default: info
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --env)      KURA_ENV="$2"; shift 2 ;;
    --bin-dir)  KURA_BIN_DIR="$2"; shift 2 ;;
    --data-dir) KURA_DATA_DIR="$2"; shift 2 ;;
    --bind)     KURA_BIND_ADDR="$2"; shift 2 ;;
...
# ---- resolve config --------------------------------------------------------
KURA_ENV="${KURA_ENV:-prod}"
case "${KURA_ENV}" in
  prod) DEFAULT_DATA="${HOME}/.kura";      DEFAULT_BIND="127.0.0.1:19191" ;;
  test) DEFAULT_DATA="${HOME}/.kura-test"; DEFAULT_BIND="127.0.0.1:19192" ;;
  *) die "KURA_ENV must be 'prod' or 'test', got '${KURA_ENV}'" ;;
esac
KURA_BIN_DIR="${KURA_BIN_DIR:-${HOME}/.local/bin}"
KURA_DATA_DIR="${KURA_DATA_DIR:-${DEFAULT_DATA}}"
KURA_BIND_ADDR="${KURA_BIND_ADDR:-${DEFAULT_BIND}}"
KURA_LOG_LEVEL="${KURA_LOG_LEVEL:-info}"
...
# KURA_CONNECTORS_DISCORD_ENABLED=true (and a valid token in config.json).
KURA_DISCORD_ENABLED="${KURA_CONNECTORS_DISCORD_ENABLED:-false}"
KURA_LOG_DIR="${KURA_DATA_DIR}/logs"
KURA_BIN="${KURA_BIN_DIR}/kura"
...
HEALTH_HOST="${KURA_BIND_ADDR%:*}"; [[ "${HEALTH_HOST}" == "0.0.0.0" || -z "${HEALTH_HOST}" ]] && HEALTH_HOST="127.0.0.1"
HEALTH_PORT="${KURA_BIND_ADDR##*:}"
HEALTH_URL="http://${HEALTH_HOST}:${HEALTH_PORT}/healthz"

info "${BOLD}Kura install${RST}  env=${KURA_ENV}  bind=${KURA_BIND_ADDR}"
echo "    binary   -> ${KURA_BIN}"
echo "    data dir -> ${KURA_DATA_DIR}"
...
# ---- build (Rust release) --------------------------------------------------
KURA_VERSION="$(git -C "${REPO_ROOT}" describe --tags --always --dirty 2>/dev/null || echo dev)"
info "building daemon (version ${KURA_VERSION}) ..."
mkdir -p "${KURA_BIN_DIR}"
TMP_BIN="$(mktemp "${KURA_BIN_DIR}/.kura.XXXXXX")"
...
chmod 0755 "${TMP_BIN}"
mv -f "${TMP_BIN}" "${KURA_BIN}"   # atomic swap; a running service picks it up on restart
trap - EXIT
info "installed binary -> ${KURA_BIN}"

# ---- data dir (daemon self-initializes config.json on first start) ---------
mkdir -p "${KURA_DATA_DIR}" "${KURA_LOG_DIR}"
...
wait_health() {
  ...
  warn "daemon did not report healthy within 30s. Check logs: ${KURA_LOG_DIR}/daemon.err.log"
  return 1
}

render() { # render <template> <dest> ; substitutes @TOKEN@
  sed -e "s|@KURA_BIN@|${KURA_BIN}|g" \
      -e "s|@KURA_ENV@|${KURA_ENV}|g" \
      -e "s|@KURA_DATA_DIR@|${KURA_DATA_DIR}|g" \
      -e "s|@KURA_BIND_ADDR@|${KURA_BIND_ADDR}|g" \
      -e "s|@KURA_VERSION@|${KURA_VERSION}|g" \
      -e "s|@KURA_LOG_LEVEL@|${KURA_LOG_LEVEL}|g" \
      -e "s|@KURA_DISCORD_ENABLED@|${KURA_DISCORD_ENABLED}|g" \
      -e "s|@KURA_LOG_DIR@|${KURA_LOG_DIR}|g" \
      "$1" > "$2"
}

if [[ "${NO_SERVICE}" == "1" ]]; then
  warn "--no-service: skipping service registration."
  info "Run manually:  KURA_ENV=${KURA_ENV} KURA_DATA_DIR=${KURA_DATA_DIR} KURA_BIND_ADDR=${KURA_BIND_ADDR} ${KURA_BIN}"
elif [[ "${OS}" == "Darwin" ]]; then
  ...
  TEMPLATE="${SCRIPT_DIR}/systemd/kura-agent.service.template"
  if [[ "${SYSTEM_MODE}" == "1" ]]; then
    UNIT="/etc/systemd/system/kura-agent.service"
    ...
    sudo systemctl enable --now kura-agent.service
    wait_health || true
  else
    UNIT_DIR="${HOME}/.config/systemd/user"
    UNIT="${UNIT_DIR}/kura-agent.service"
    ...
    systemctl --user enable --now kura-agent.service
...
# ---- client guidance -------------------------------------------------------
echo
info "${BOLD}Done.${RST} The daemon listens on ${KURA_BIND_ADDR}."
PATH_HINT=""
case ":${PATH}:" in *":${KURA_BIN_DIR}:"*) ;; *) PATH_HINT=" (add ${KURA_BIN_DIR} to PATH)";; esac
cat <<EOF

Connect a client (clients default to the test port 19192 — point them at this daemon):

  TUI:   KURA_DAEMON_URL=http://${HEALTH_HOST}:${HEALTH_PORT} kura-tui
         (build once: cd ${REPO_ROOT}/crates && cargo build --release -p kura-tui)
...
$( [[ "${NO_SERVICE}" == "1" ]] && echo "(no service was registered: --no-service)" && exit 0
   ...
   [[ "${OS}" == "Darwin" ]] \
    ...
    && echo "  logs:   tail -f ${KURA_LOG_DIR}/daemon.err.log" \
    && echo "  stop:   launchctl bootout gui/$(id -u)/${SERVICE_LABEL}" \
    || { if [[ "${SYSTEM_MODE}" == "1" ]]; then \
           echo "  status: systemctl status kura-agent"; echo "  logs:   journalctl -u kura-agent -f"; \
         else \
           echo "  status: systemctl --user status kura-agent"; echo "  logs:   journalctl --user -u kura-agent -f"; \
         fi; } )
