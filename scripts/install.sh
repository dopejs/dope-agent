...
echo "building dope-cli (release)..."
cargo build --release -p dope-cli --manifest-path "${RS_DIR}/Cargo.toml"

mkdir -p "$BIN_DIR"
install -m 0755 "${RS_DIR}/target/release/dope-cli" "${BIN_DIR}/dope"
echo "installed ${BIN_DIR}/dope"

mkdir -p "$DATA_DIR"
echo "data directory: $DATA_DIR (created if absent; existing state untouched)"

VERSION_LINE="$("${BIN_DIR}/dope" --version 2>/dev/null || true)"
...
next steps:
  DOPE_ENV=$ENV_NAME dope          # start the daemon ($ADDR)
