...
if [[ -n "${KURA_HOSTED_HEALTH_COMMAND:-}" ]]; then
  sh -c "$KURA_HOSTED_HEALTH_COMMAND" >/dev/null
else
  scripts/check-daemon-health.sh "$KURA_DAEMON_ADDR"
fi
...
if [[ -f "$KURA_DATA_DIR/daemon.sqlite" ]]; then
  ...
  if sqlite3 "$KURA_DATA_DIR/daemon.sqlite" "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'r39_tenants';" | grep -q r39_tenants; then
    R39_TENANT_COUNT="$(sqlite3 "$KURA_DATA_DIR/daemon.sqlite" 'SELECT COUNT(*) FROM r39_tenants;')"
...
{
  "upgradeEvidenceId": "upgrade_postflight_${KURA_HOSTED_RUN_ID}",
