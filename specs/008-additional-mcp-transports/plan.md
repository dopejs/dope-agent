...
`websocket` MCP server running in `KURA_ENV=test`.
...
**Storage**: SQLite daemon state for MCP server documents, lifecycle state, tool exposure, tool-call history, and event history; additive transport capability and recovery truth persisted inside existing MCP server documents and event records; environment-scoped config and secret material under `~/.kura-test` or `~/.kura`
**Testing**: `go test ./internal/mcp ./internal/api ./internal/app ./internal/store ./internal/contracts`, `make daemon-contract-test`, `go test ./...`, plus one manual `KURA_ENV=test` websocket verification workflow
**Target Platform**: macOS/Linux local daemon in `KURA_ENV=test` by default, with a repo-owned websocket MCP helper server reachable over localhost for deterministic verification
...
- Verification and observability: PASS. The plan requires targeted daemon tests, contract coverage, reconnect and restore regressions, and a manual `KURA_ENV=test` websocket workflow. Recovery and mismatch truth are explicit outputs, not hidden implementation details.
- Environment and secrets: PASS. Planning assumes `KURA_ENV=test` and localhost verification. Websocket auth stays bound to existing MCP secret refs with redacted projection and no process-environment-only secret path.
