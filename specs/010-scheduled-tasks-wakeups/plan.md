...
events, and restart-safe recovery in `KURA_ENV=test`.
...
**Testing**: `go test ./internal/scheduler ./internal/api ./internal/store ./internal/app ./internal/contracts`, `make daemon-contract-test`, `go test ./...`, plus one manual `KURA_ENV=test` verification covering one-time dispatch and recurring pause/resume behavior
...
- Verification and observability: PASS. The design requires targeted scheduler, store, API, app recovery, and contract coverage plus one manual `KURA_ENV=test` run. Operator-visible events and resource history replace raw-log reconstruction for dispatch failure, downstream failure, overlap skip, retry exhaustion, and restart catch-up.
- Environment and secrets: PASS. Local work stays in `KURA_ENV=test`, schedule dispatch reuses existing run/workflow approval and policy semantics, and the roadmap adds no new secret-bearing execution path.
...
- `cd daemon && go test ./...`
- manual `KURA_ENV=test` one-time schedule creation, pre-fire inspection, due-time
