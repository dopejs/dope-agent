...
- Artifact directory: `/root/kura-agent-r41-artifacts`
- Full soak report: `/root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.json`
- Full soak log: `/root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.log`
...
```bash
KURA_DATA_DIR=/root/.kura-r41-soak
KURA_SOAK_BRANCH_OR_VERSION=5ad95ba
KURA_SOAK_REPORT=/root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.json
```

Recorded PID files:

```bash
/root/kura-agent-r41-artifacts/daemon-5ad95ba.pid
/root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.pid
```
...
ssh -o BatchMode=yes zentalk-1 'ps -eo pid,ppid,comm,args | grep -E "run-soak|daemon-run-test|cmd/kura|/tmp/go-build" | grep -v grep || true'
ssh -o BatchMode=yes zentalk-1 'curl -fsS http://127.0.0.1:19192/healthz || true'
ssh -o BatchMode=yes zentalk-1 'tail -80 /root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.log'
ssh -o BatchMode=yes zentalk-1 'test -f /root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.json && cat /root/kura-agent-r41-artifacts/roadmap39-full-5ad95ba.json'
ssh -o BatchMode=yes zentalk-1 'tail -120 /root/kura-agent-r41-artifacts/daemon-5ad95ba.log'
```
