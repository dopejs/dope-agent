...
- Go package `internal/foo` → crate `kura-foo` at `rs/foo`. Folded packages:
  `tenantctx` and `auth` live inside `kura-identity`; `imtypes` inside `kura-im`.
- Types: `serde` with `#[serde(rename_all = "camelCase")]` to match Go JSON
  tags. Shared IDs (`TenantId`, `ThreadId`, `RunId`, `ProfileId`) come from
  `kura-protocol`; add new UUIDv7 IDs there via `define_id!`.
...
- **Persistence inversion**: Go's `internal/store` converges on every domain
  package (typed DAOs). In Rust, `kura-store` owns the SQLite connection,
...
| 8 surface | api(26.3k, split by route family), app(2401), daemon binary wiring, contract-test parity | done (2026-08-16 closure) — the original "16 route families" claim was an overstatement: the Go daemon served 22+ mux patterns. A route-table audit found 14 unmounted families (triage, routines, catalog, execution profiles, evidence bundles, launch-gate, config, sessions, llm dispatches incl. SSE, sandboxes, capabilities, connectors incl. ingress, policy approvals, providers); all are now ported, `protected()` is attached to the production router with Go's unauthenticated allowlist, and `tests/route_parity.rs` diffs the mounted routes against the recorded Go table as a permanent gate. Residual (Roadmap 75): per-handler tenant-context integration (hosted credential permission checks, tenant-scoped list/persist variants) is documented per module and inert until identity-resolved tenant contexts are exercised. |
